...
func radiate_value_around_position(position: Vector2, radius: float, curve: mCurve, 
	...
	for y in range(min_y, max_y):
		for x in range(min_x, max_x):
			...
			grid[x + y * width] = curve.calculate_value(distance) * magnitude
...
func add_grid_at_pos(other_grid: FlatGrid, position: Vector2, magnitude := 1.0) -> void:
	...
	for y in range(min_y, max_y):
		for x in range(min_x, max_x):
			...
			var value: float = other_grid.grid[other_x + other_y * other_grid.width]
...
func grid_to_string() -> String:
	...
	for y in range(height):
		for x in range(width):
			result += str(grid[x + y * width]).pad_decimals(1) + " "
		result += "\n"
	return result
