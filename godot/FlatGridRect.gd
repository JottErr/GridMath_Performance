class_name FlatGridRect
extends RefCounted


## Rectangle that stores position, width and height of the grid in cells as ints
var rect: Rect2i
var grid: Array[float] ## 1D Array that holds the values for each cell of a 2D grid.
...
## index.
func _create_new_grid() -> Array[float]:
	var new_grid: Array[float] = []
	new_grid.resize(rect.get_area())
	return new_grid
...
func radiate_value_around_position(position: Vector2, radius: float, curve: mCurve, 
	...
	for x in range(lim_box.position.x, lim_box.end.x):
		for y in range(lim_box.position.y, lim_box.end.y):
			...
			grid[x + y * rect.size.x] = curve.calculate_value(distance) * magnitude
...
func add_grid_at_pos(other_grid: FlatGridRect, position: Vector2i, magnitude := 1.0) -> void:
	...
	for x in range(lim_box.position.x, lim_box.end.x):
		for y in range(lim_box.position.y, lim_box.end.y):
			...
			var value: float = other_grid.grid[other_x + other_y * other_grid.rect.size.x]
			grid[x + y * rect.size.x] += value * magnitude
...
func grid_to_string() -> String:
	...
	for y in range(rect.size.y):
		for x in range(rect.size.x):
			result += str(grid[x + y * rect.size.x]).pad_decimals(1) + " "
		result += "\n"
	return result
