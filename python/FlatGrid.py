...
class FlatGrid():
	...
	def __init__(self, width, height):
		...
		self.grid = [0.0 for _ in range(self.height * self.width)]
	...
	def radiate_value_around_position(self, position, radius, curve, magnitude = 1.0):
		...
		for y in range(int(min_y), int(max_y)):
			for x in range(int(min_x), int(max_x)):
				...
				self.grid[x + y * self.width] = curve.calculate_value(distance) * magnitude
	...
	def add_grid_at_pos(self, other_grid, position, magnitude = 1.0):
		...
		for y in range(min_y, max_y):
			for x in range(min_x, max_x):
				other_x = x - start_x
				other_y = y - start_y
				value = other_grid.grid[other_x + other_y * other_grid.width]
				self.grid[x + y * self.width] += value * magnitude

	def __str__(self):
		result = ""
		for y in range(self.height):
			for x in range(self.width):
				result += f"{self.grid[x + y * self.width]:.1f} "
			result += "\n"
		return result